...
bcontent.accessor(function (d) {
    return d.Energy;
});
    var svg = d3.select("bcontent").append("svg").attr("width", width + margin.left + margin.right)
...
//hover effect params
var innerRadius = bcontent.innerRadius();
bcontent.numSegments();
var segmentHeight = bcontent.segmentHeight();

// call mouseover event
bcontent.on("customHover", mouseover(svg, index_one, innerRadius, numSegments, segmentHeight, dataDisplayed));
...
function swapDataset(passedData, radiallabels) {
    bcontent.radialLabels(radiallabels)
         .domain([0, 2, 4, 6])
         .range(range_blue);
	d3.select("bcontent")
        .datum(passedData)
...
//update viz
function updatebcontent(passedData,radiallabels){
	bcontent.radialLabels(radiallabels)
         .domain([0,2,4,6])
         .range(range_blue);
	d3.select("bcontent")
...
function mouseover(svg,index,innerRadius,numSegments,segmentHeight,passedData){
 svg.selectAll("path.segments"+index)
    .on("mouseover", function(d, i) {
        ...
        if(data.length < 400){
            d3.selectAll("path.segments"+index) //.arc indicates segment
                .transition()
                .duration(200)
                .attr("d",d3.svg.arc()
                      .innerRadius(ir)
            ...
            d3.selectAll("path.segments"+index)
                .transition()
...
function drawSlider(passedData){
	...
	$('#1slider-time').html(dt_from);
	$('#1slider-time2').html(dt_to);
    ...
    $("#1slider-range").slider({
        ...
        slide: function (e, ui) {
            ...
            $('#1slider-time').html(formatDT(dt_cur_from));
            var dt_cur_to = new Date(ui.values[1]*1000);           
            $('#1slider-time2').html(formatDT(dt_cur_to));
        },
        stop: function(e, ui) { 
            var html = $('#1slider-time').html();
            var html2 = $('#1slider-time2').html();
...
function filterData(start,end){
	...
	updatebcontent(filtered,radial_labels);
	dataDisplayed = filtered;
	segmentHeight = (500 - 2 * innerRadius) / (2 * radial_labels.length);// should make this dynamic
    index=0;
    bcontent.on("customHover", mouseover(svg,index,innerRadius,numSegments,segmentHeight,dataDisplayed));
}
...
function circularHeat() {
    ...
    var numSegments = 24;
    var innerRadius = 80; 
	// the length of the radius of the donut bcontent is: (width-innerRadius*2) / 2
	// the length of each segment is: the radius of the donut bcontent / the number of layers
    ...
    function bcontent(selection) {
        selection.each(function (data) {
            ...
            svg.select('g.circular-heats').attr('id','bcontent'+_index);
            ...
            .attr("fill", function (d) {return color(accessor(d));});
                ...
                segments
					...
					.attr("id", function (d) {
						...
						return "segments-" + d.Day + "-" + timeCleaned+"-"+id;
					})
					.attr("class", function (d) {
						return "segments-" + d.Day + " segments"+_index
					})
					.attr("d", d3.svg.arc().innerRadius(ir).outerRadius(or).startAngle(sa).endAngle(ea))
            ...
            svg.select('g.labels').append("def")
                .append("path")
                .attr("id", "segments-label-path-" + _index)
            ...
            svg.select('g.labels').selectAll("text")
                ...
                .append("text")
                .append("textPath")
                .attr("xlink:href", "#segments-label-path-" + _index)
...
//     	console.log(innerRadius + floor * segmentHeight);
    ...
    bcontent.margin = function (_) {
        if (!arguments.length) return margin;
        margin = _;
        return bcontent;
    };
    
    bcontent.width = function (_) {
        if (!arguments.length) return width;
        margin = _;
        return bcontent;
    };

    bcontent.innerRadius = function (_) {
        if (!arguments.length) return innerRadius;
        innerRadius = _;
        return bcontent;
    };

    bcontent.numSegments = function (_) {
        if (!arguments.length) return numSegments;
        numSegments = _;
        return bcontent;
    };

    bcontent.segmentHeight = function (_) {
        if (!arguments.length) return segmentHeight;
        segmentHeight = _;
        return bcontent;
    };

    bcontent.domain = function (_) {
        if (!arguments.length) return domain;
        domain = _;
        return bcontent;
    };

    bcontent.range = function (_) {
        if (!arguments.length) return range;
        range = _;
        return bcontent;
    };

    bcontent.radialLabels = function (_) {
    ...
    bcontent.segmentLabels = function (_) {
    ...
    bcontent.accessor = function (_) {
        if (!arguments.length) return accessor;
        accessor = _;
        return bcontent;
    };
    
	bcontent.offset = function(value) {
			if (!arguments.length) return offset;
			offset = value;
			return bcontent;
	};
	
	bcontent._index = function(value) {
			if (!arguments.length) return _index;
			_index = value;
			return bcontent;
	};

	d3.rebind(bcontent, dispatch, "on");
    return bcontent;
